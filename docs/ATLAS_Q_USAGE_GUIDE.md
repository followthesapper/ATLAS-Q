...
Practical tutorials for GPU-accelerated quantum simulation, tensor networks, and period-finding.
...
factors = qc.factor_number(15)
assert factors == [3, 5] or factors == [5, 3]

# Test tensor networks
adaptive = get_adaptive_mps()
mps = adaptive['AdaptiveMPS'](10, bond_dim=8, device='cpu')

print("✓ ATLAS-Q v0.5.0 verified!")
```
...
```python
from atlas_q import ProductState
...
### 5.4 Adaptive MPS with GPU Acceleration

ATLAS-Q v0.5.0 includes GPU-accelerated adaptive MPS with custom Triton kernels:

```python
from atlas_q import get_adaptive_mps
import torch

# Get AdaptiveMPS class
adaptive = get_adaptive_mps()
AdaptiveMPS = adaptive['AdaptiveMPS']

# Create GPU-accelerated MPS
mps = AdaptiveMPS(n_qubits=20, bond_dim=32, device='cuda')

# Apply gates with automatic adaptive truncation
H = torch.tensor([[1,1],[1,-1]], dtype=torch.complex64, device='cuda') / torch.sqrt(torch.tensor(2.0, device='cuda'))
...
mps_module = get_adaptive_mps()

# Build Ising Hamiltonian
H = mpo_module['MPOBuilder'].ising_hamiltonian(
    n_sites=10, J=1.0, h=0.5, device='cuda'
)

# Initial state
mps = mps_module['AdaptiveMPS'](10, bond_dim=16, device='cuda')
...
# Run time evolution
tdvp = tdvp_module['TDVP2Site'](H, mps, config)
...
H = mpo_module['MPOBuilder'].heisenberg_hamiltonian(
    n_sites=6, device='cuda'
)
...
### 5.7 Stabilizer Backend

Fast Clifford circuit simulation with automatic MPS handoff:
...
# Create hybrid simulator (automatic backend switching)
sim = stab['HybridSimulator'](n_qubits=10, use_stabilizer=True)

# Apply Clifford gates (uses fast stabilizer backend)
sim.h(0)
sim.cnot(0, 1)
sim.s(2)
...
print(f"Current mode: {stats['mode']}")  # 'stabilizer'

# Apply non-Clifford gate (automatically switches to MPS)
sim.t(0)

stats = sim.get_statistics()
print(f"Current mode: {stats['mode']}")  # 'mps'
print(f"20× speedup on Clifford-heavy circuits!")
```

### 5.8 Noise Models

NISQ simulation with realistic noise:

```python
from atlas_q import get_noise_models, get_adaptive_mps

noise_module = get_noise_models()
mps_module = get_adaptive_mps()

# Create noise model
noise = noise_module['NoiseModel'].depolarizing(p1q=0.001, p2q=0.01)

# Create noisy simulator
mps = mps_module['AdaptiveMPS'](10, bond_dim=16, device='cuda')
noise_applicator = noise_module['StochasticNoiseApplicator'](noise, seed=42)
...
for q in range(10):
    mps.apply_single_qubit_gate(q, H)
    noise_applicator.apply_noise_after_gate(mps, [q])  # Add depolarizing noise

print(f"Noisy simulation complete")
print(f"Approximate fidelity tracking supported")
```
...
```python
import numpy as np
from atlas_q.tools_qih import periodic_mixture_features
...
```python
from atlas_q.tools_qih import qih_pat
...
```python
from atlas_q.tools_qih import tn_core
...
- **[GitHub Issues](https://github.com/your-org/ATLAS-Q/issues)** - Report bugs

---

**Happy Simulating!** 🎉

*For questions: https://github.com/your-org/ATLAS-Q/issues*
