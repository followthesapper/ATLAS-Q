...
def mps_apply_2q(cores, i, j, U, chi_max=None, eps=1e-10, svd_driver='gesvdj', 
                 adaptive=False, tol=1e-4, ai_predictor=None):
    """
    ...
    try:
        ...
        SVD_EVENT_LOGGER.log(
            ...
            tol=float(tol), adaptive=bool(adaptive),
            entangler=meta_entangler,
            total_weight=float(total_weight.detach().cpu().item() if hasattr(total_weight,'detach') else float(total_weight)),
            S_top128=Sv,
