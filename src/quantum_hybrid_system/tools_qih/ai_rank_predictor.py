        ...
        for epoch in range(epochs):
            ...
            if val_loader is not None:
                ...
                if val_loss < best_val_loss:
                    ...
                    print(f"  ⭐ Epoch {epoch+1}/{epochs} - Train Loss: {avg_loss:.6f}, Val Loss: {val_loss:.6f} (BEST), LR: {scheduler.get_last_lr()[0]:.6f}")
                else:
                    print(f"  Epoch {epoch+1}/{epochs} - Train Loss: {avg_loss:.6f}, Val Loss: {val_loss:.6f}, LR: {scheduler.get_last_lr()[0]:.6f}")
            else:
                print(f"  Epoch {epoch+1}/{epochs} - Train Loss: {avg_loss:.6f}, LR: {scheduler.get_last_lr()[0]:.6f}")
