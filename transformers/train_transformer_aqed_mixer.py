...
# -------------------------
# Synthetic dataset (same as baseline)
# -------------------------
...
class AQEDMixLite(nn.Module):
    """
    Token mixer influenced by AQED: applies K layers of random disjoint 2x2 rotations
    across sequence positions. Use pair_frac to control coverage per layer.
    Cost ~ O(K · pair_frac · L · D). Fully differentiable and CUDA-friendly.
    """
    def __init__(self, d_model, depth=3, pair_frac=0.5, channel_mix=True):
        super().__init__()
        self.depth = depth
        self.pair_frac = pair_frac
        self.theta = nn.Parameter(torch.zeros(depth))  # learnable per-layer angles
        self.channel = nn.Sequential(
            nn.Linear(d_model, 2*d_model, bias=False),
            nn.GELU(),
            nn.Linear(2*d_model, d_model, bias=False),
        ) if channel_mix else nn.Identity()

    @torch.no_grad()
    def _make_pairs(self, L, device):
        perm = torch.randperm(L, device=device)
        P = (L // 2)
        pairs = perm[: 2*P].view(-1, 2)  # [P, 2]
        keep = max(1, int(self.pair_frac * pairs.size(0)))
        return pairs[:keep]  # [K, 2]
    ...
    def forward(self, x):
        ...
        for k in range(self.depth):
            pairs = self._make_pairs(L, x.device)  # [K, 2]
            a = y[:, pairs[:,0], :]  # [B, K, D]
            b = y[:, pairs[:,1], :]  # [B, K, D]
            c, s = torch.cos(self.theta[k]), torch.sin(self.theta[k])
            a2 =  c * a + s * b
            b2 = -s * a + c * b
            y = y.clone()
            y[:, pairs[:,0], :] = a2
...
# -------------------------
# Hybrid Transformer (alt attention/mixer)
# -------------------------
...
class MixerBlock(nn.Module):
    def __init__(self, d_model, d_ff, mixer_depth=3, pair_frac=0.5, dropout=0.0):
        super().__init__()
        self.mixer = AQEDMixLite(d_model, depth=mixer_depth, pair_frac=pair_frac, channel_mix=True)
...
class TinyHybridLM(nn.Module):
    def __init__(self, vocab_size, d_model=384, n_layers=6, n_heads=6, d_ff=1536, dropout=0.0,
                 max_len=8192, mixer_depth=3, pair_frac=0.5):
        super().__init__()
        ...
        for i in range(n_layers):
            if i % 2 == 0:
                blocks.append(AttnBlock(d_model, n_heads, d_ff, dropout))
            else:
                blocks.append(MixerBlock(d_model, d_ff, mixer_depth=mixer_depth, pair_frac=pair_frac, dropout=dropout))
...
# -------------------------
# Train loop (same metrics as baseline)
# -------------------------
@dataclass
class Config:
    vocab_size: int = 256
...
def train(cfg: Config):
    ...
    model = TinyHybridLM(
        cfg.vocab_size, cfg.d_model, cfg.n_layers, cfg.n_heads, cfg.d_ff, cfg.dropout,
        max_len=8192, mixer_depth=cfg.mixer_depth, pair_frac=cfg.pair_frac
    )
    model.to(device)
...
def main():
    ...
    p.add_argument("--mixer_depth", type=int, default=3)
    p.add_argument("--pair_frac", type=float, default=0.5)
    ...
    p.add_argument("--log_csv", type=str, default="aqed_stats.csv")
    args = p.parse_args()
